(function () {
  ...
  // feeds side by side, each tied to its own RSS Pack you pick when you
  ...
  let readColumns = null; // array of RSS Pack slugs (or null = unassigned), lazy-loaded
  ...
  // Resolves a dispatch back to a source an RSS Pack can actually hold. Custom
  ...
  // null).
  //
  // Label deliberately says "Follow [outlet]", not "+ RSS Pack" — this
  // control adds the SOURCE (the whole outlet/individual/feed) to a
  // RSS Pack, not just this one article. "+ RSS Pack" alone read as if it
  ...
  // non-official RSS Packs (see quickRemoveContext below), every item shown
  // is — by construction, since it was fetched FROM that RSS Pack's own
  ...
  // RSS Packs (an RSS Pack is a collection of SOURCES; a save is a single ITEM).
  ...
  // Removes a source from the RSS Pack the reader is currently viewing, with
  ...
  async function quickRemoveFromActiveSpray(source, slug, btnEl, onSuccess) {
    ...
    try {
      ...
      if (result.added) {
        ...
        // this RSS Pack's own list. Toggle it back off rather than leave a
  ...
  // ---------------------------------------------------------------
  // READ — the RSS reader, built on RSS Packs. Maps to /api/dispatches.
  ...
  // own, non-official RSS Packs via the toggle bar — {slug, name}. Read by
  // sprayAddButton() to flip "+ RSS Pack" (open the picker) into a one-tap
  // "− Remove" for every card shown, since everything in view got there
  // BY BEING a member of that RSS Pack. Recomputed each fetchReadItems()
  // call; cleared whenever "All", multiple RSS Packs, or a built-in slot
  // (__youtube/__bluesky) is active, or the single active RSS Pack isn't
  // one the reader actually owns (e.g. someone else's public RSS Pack).
  let quickRemoveContext = null;

  // Shared official flagship RSS Pack slug — matches server.mjs's
  ...
  // Multi-select: a reader can view several RSS Packs combined at once.
  ...
  // SOURCE!-only display shortener: the shared flagship RSS Pack's real
  ...
  // the full name is too wordy. Any other RSS Pack's name passes through
  // untouched.
  ...
  // fetches a real RSS Pack (not the built-in __youtube/__bluesky slots) —
  // slug -> {name, is_owner, is_official}. Read by fetchReadItems() to
  // decide whether the single active RSS Pack qualifies for quick-remove.
  ...
  async function fetchItemsForSprayKey(key) {
    ...
    try {
      ...
      if (key === OFFICIAL_NEWS_SLUG) {
        // The flagship RSS Pack is just a live mirror of the default Wire —
  ...
  // one or more RSS Pack slugs and/or the built-in "__youtube" slot — is
  // fetched in parallel and merged: deduped by link (so the same story
  // showing up in two selected RSS Packs doesn't double), sorted newest
  // first. A key that fails (RSS Pack deleted/went private) is silently
  ...
  async function fetchReadItems() {
    ...
    results.forEach((r, i) => {
      if (r.status === "fulfilled") {
        ...
        console.error(`RSS Pack toggle: "${keys[i]}" failed to load and was dropped from the merge:`, r.reason);
      }
    });
    ...
    // Exactly one real (non-built-in) RSS Pack in view, and the reader owns
    ...
    // selected, a built-in slot, or an RSS Pack that isn't theirs) reverts
    // every card back to the normal "+ RSS Pack" add flow.
  ...
  // its own RSS Pack. Fully independent of the Read toggle bar's selection
  // (activeSprayKeys) — a column's RSS Pack choice is a separate, per-column
  ...
  async function loadReadColumnItems(slug, i) {
    ...
    try {
      ...
      // The reader may have changed this column's RSS Pack while the fetch
  ...
  // Read-tab RSS Pack bar, and (via "+ New RSS Pack") the guided Create flow.
  ...
  // location field on an RSS Pack instead of a topic tag at all.
  ...
  //   YOUR SOURCES  — everything you can build an RSS Pack from, searchable,
  //     each one drills into a detail view (which of your RSS Packs it's
  //     already in) that ends at the same "+ RSS Pack" picker used from a
  //     post — this is the one general way to EDIT an RSS Pack's contents.
  ...
  // ----- RSS Packs shelf: Headlines (starter pack) + your own RSS Packs + New -----
  ...
  // toggle (adds/removes the official RSS Pack from the reader's own
  // Read-toggle bar — a genuine, reversible action) plus "Copy and
  // customize" (creates a brand-new, independently editable RSS Pack
  // pre-filled with whatever Headlines currently carries, since the
  // official RSS Pack itself has no stored source list to edit — it
  // auto-syncs to the live Wire). An owned RSS Pack gets rename,
  // public/private, its source list with add/remove, and its follower
  // (clone) count — this is the one place an RSS Pack's OWN contents get
  ...
  async function renderSources() {
    ...
    try {
      ...
      // Sources is for building RSS Packs out of Organizations — Individuals (Bluesky-only
      ...
      // ----- LEFT: reordering your Read toggle (which RSS Packs show up as
      ...
      // Create-a-RSS Pack flow (name it, pick a topic, add sources, save).
      ...
      // into the unfiltered wire automatically) and offers the RSS Pack
  ...
  async function addCustomSourceDirect(feedUrl, input, btn) {
    ...
    try {
      ...
      // an existing RSS Pack (or start a new one) via the same picker every
      // other "+ RSS Pack" entry point in the app already uses.
  ...
  // reader's own RSS Packs already carry it, ending in the same "+ RSS Pack"
  ...
  // ----- "Add to an RSS Pack" quick picker — opens straight off a post (card
  // or reader pane). Check/uncheck which of your own RSS Packs this source
  ...
  // to "editing an RSS Pack is hard" — it's the one place a reader can add a
  // source to an EXISTING RSS Pack at all; the guided Create flow only ever
  // builds new ones. -----
  ...
  function closeSprayPicker() {
    ...
    // "in your RSS Packs" membership after a toggle.
  ...
  // devices, same pattern as the Read-tab RSS Pack bar and Edit Layout.
  // ---------------------------------------------------------------
  ...
  // RSS Packs/custom-sources management.
  // ---------------------------------------------------------------
  ...
  // Distinct from RSS Packs: an RSS Pack is a collection of SOURCES a reader
  ...
  // #9, a reader can also clone the flagship RSS Pack for real later
